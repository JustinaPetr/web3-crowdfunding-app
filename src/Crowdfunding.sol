...
contract Crowdfunding {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Not the ownder");
        _;
    }

    modifier campaignOpen () {
        require(state == CampaignState.Active, "Campaign is not agive");
        _;
    }
    ...
    function fund (uint256 _tierIndex) public payable campaignOpen notPaused {
        require(_tierIndex < tiers.length, "Invalide tier.");
    ...
    function withdraw () public onlyOwner {
        checkAndUpdateCampaignState();
        require(state == CampaignState.Successful, "Campaing is not yet successfull");
    ...
    function refund() public {
        checkAndUpdateCampaignState();
        require(state == CampaignState.Failed, "Refunds are not available");
